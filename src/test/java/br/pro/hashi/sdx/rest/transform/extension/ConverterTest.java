...
class ConverterTest {
	...
	@Test
	void getsSourceType() {
		when(reflector.getSpecificType(eq(Converter.class), eq(0), any())).thenReturn(Double.class);
	...
	@Test
	void getsTargetType() {
		when(reflector.getSpecificType(eq(Converter.class), eq(1), any())).thenReturn(Integer.class);
		c = new ConcreteConverter();
